...
/************************************Variable define******************************************/
static INT8U sAdTemp[NUMTEMP][FILTER_NUM];                           /* 温度数据缓冲区 */
/*
************************************************************************************************
*  Function:         ADCInit()
*  Description:      ADC初始化
...
void AdcTemp(const INT8U *p_ch,INT8U *p_temp)
{
    ...
    for(i = 0; i < NUMTEMP; i++)
    {
        ...
        if(i <= NUMTEMP)                                       /* NTC采样线 */
        {
            res = (INT32U)(1000) * result / (1024 - result);   /* 计算阻值 */
            ...
            {
                while((res < ResTemp[n]) && (n < 160))         /* 由阻值查阻值-温度表, 表内偏移-40℃ */
                {
                    n++;                                           
                }
                if((res - ResTemp[n]) > (ResTemp[n-1] - res))  /* 四舍五入 */
                {
                	  n--;
                }
            }
        ...
        {
            res = (INT32U)(100) * result / (1024 - result);    /* 计算阻值 */
            ...
            {
                while((res < BDResTemp[n]) && (n < 190))                 /* 由阻值查阻值-温度表, 表内偏移-40℃ */
                {
                    n++;                                           
                }
                if((res - BDResTemp[n]) > (BDResTemp[n-1] - res))          /* 四舍五入 */
                {
                	  n--;
                }
            }
    ...
    if(sAdcTemp_first_rap == FALSE)
    {
        for(i = 0; i < NUMTEMP; i++)
        {        
            p_temp[i] = AdcFilter(&sAdTemp[i][0]);                 /* 温度数据滤波 */   
        }
    }
    else
    {
        ...
        {       
            ...
            p_temp[i] = sum / sAdcTemp_temp_location;         /* 第一次,直接取平均值 */
        }         
    }                  
}
/*
